...
export class ProductAutomatorBooking extends AbsProductAutomator<ProductBooking> {
 ...
 async planGenerate(time: TimeMillies = Date.now()): Promise<ProductBooking[]> {
   ...
   bookings = bookings.filter((booking) => {
    ...
    if (
     this.exceptedDayOfWeeks.some(
      (exceptDay) => DayOfWeekToInt[exceptDay] === new Date(date).getDay()
     )
    ) {
     return false;
    }
  ...
  const products = pullAllWith(
   ...
   (prodOrigin, prodExisting) => {
    return (
     prodOrigin.automatorInfo?.hashCode === prodExisting.automatorInfo?.hashCode
    );
   }
  );
  return products;
